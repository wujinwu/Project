...
## 访问远程服务器
1.配置网络,实现客户机与服务器互通
2.服务器开启允许被远程访问:桌面右击属性-远程设置
3.客户机：开始--运行--输入mstsc打开远程连接工具
4.在mstsc工具上输入服务器的Ip并点击确定
5.输入服务器账号密码
6.如果非管理员登录，则需要服务器管理员将用户加入远程访问组 net localgroup "Remote Desktop Users"

### telnet 命令行远程控制
1.服务器:点击开始运行,输入services.msc，打开telnet服务
2.保证服务器能ping通
3.客户机:cmd中输入telnet 服务器地址,输入用户名，密码,连接到服务器。
4.netstat -an 查看本机所有开放的端口
telnet端口:23
远程桌面协议RDP端口:3389

### 破解系统密码
1.利用win7漏洞破解密码,5次shift漏洞,掉包为cmd
2.强制关机，出现正常启动，和启动修复,选中启动修复,---查看问题详细信息,拉到最后,点第二个链接,进入系统，把cmd改成sethc,然后关闭电脑，进入，按5次shift打开cmd然后创建管理员账号,利用创建账号进入系统
3.net user us "" 清空us用户密码

### NTFS权限,NTFS是文件系统,windows用
1.NTFS权限，给文件和文件夹设置不同的权限，给不同的用户访问
2.格式化就是在制作文件系统
3.磁盘配合，为每一个用户分配一个空间
4.设置访问权限
5.加密文件系统
#### 修改NTFS权限
1.建议一个文档，分配给A和B，A和B权限不同
2.新建用户A和B
3.给A分配读取权限
